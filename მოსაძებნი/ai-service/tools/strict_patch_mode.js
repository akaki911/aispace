...
class StrictPatchMode {
  ...
  ensureBackupDir() {
    try {
      ...
      try {
        fsSync.accessSync(gitignorePath);
      } catch {
        fsSync.writeFileSync(gitignorePath, '# Assistant backup files\n*\n!.gitignore\n');
      }
  ...
  findProjectRoot(filePath) {
    ...
    while (currentDir !== rootDir) {
      ...
      const markers = ['package.json', 'tsconfig.json', 'vite.config.js', 'vite.config.mts'];
