...
// Enhanced query classification for multi-stage RAG routing
function classifyQuery(query) {
  const lowerQuery = query.toLowerCase();

  // Multi-stage thematic analysis patterns
  const thematicPatterns = {
    // Full project overview - requires multiple stages
    'full_project_analysis': [
      'მთელი საიტის შესახებ', 'სრული ინფორმაცია', 'დეტალური აღწერა მთელი პლატფორმის',
      'ყველაფერი რაც არის', 'პროექტის სრული მიმოხილვა', 'სისტემის სრული აღწერა'
    ],

    // Project structure - directory focused
    'project_structure': [
      'საიტის სტრუქტურა', 'პროექტის სტრუქტურა', 'დირექტორიების აღწერა',
      'ფოლდერების დანაწილება', 'არქიტექტურული სტრუქტურა', 'ფაილების ორგანიზაცია'
    ],

    // Component analysis - specific modules
    'component_analysis': [
      'კომპონენტების ანალიზი', 'მოდულების აღწერა', 'სერვისების ფუნქციონალი',
      'React კომპონენტები', 'Backend სერვისები', 'Frontend კომპონენტები'
    ],

    // Technical deep dive - code focused
    'technical_deep_dive': [
      'კოდის დეტალური ანალიზი', 'ფუნქციების აღწერა', 'ტექნიკური იმპლემენტაცია',
      'API endpoints', 'database schema', 'ბიზნეს ლოგიკა'
    ],

    // Quick overview - summary focused
    'quick_overview': [
      'მოკლე აღწერა', 'bullet point ინფორმაცია', 'ძირითადი ფუნქციები',
      'შეჯამება', 'overview', 'summary'
    ]
  };

  // Technical query patterns - enhanced
  const technicalPatterns = {
    'specific_file_analysis': [
      'ფაილის ანალიზი', 'კონკრეტული კომპონენტი', 'რომელი ფაილი',
      'specific file', 'particular component', 'individual module'
    ],

    'error_debugging': [
      'ერორი', 'შეცდომა', 'ბაგი', 'პრობლემა', 'არ მუშაობს',
      'error', 'bug', 'issue', 'problem', 'debug'
    ],

    'implementation_help': [
      'როგორ გავაკეთო', 'დამეხმარეთ', 'იმპლემენტაცია',
      'how to implement', 'help with', 'assistance'
    ]
  };
  ...
  // Check for multi-stage thematic analysis
  for (const [type, patterns] of Object.entries(thematicPatterns)) {
    if (patterns.some(pattern => lowerQuery.includes(pattern))) {
      return {
        category: 'thematic',
        type: type,
        requiresMultiStage: ['full_project_analysis', 'project_structure'].includes(type),
        priority: type === 'full_project_analysis' ? 'highest' : 'normal'
      };
    }
  }

  // Check for technical queries
  for (const [type, patterns] of Object.entries(technicalPatterns)) {
    if (patterns.some(pattern => lowerQuery.includes(pattern))) {
      return {
        category: 'technical',
        type: type,
        requiresCodeRetrieval: true,
        priority: 'normal'
      };
    }
  }
  ...
  if (greetingPatterns.some(pattern => lowerQuery.includes(pattern))) {
    return { category: 'greeting', type: 'simple_greeting', requiresMinimalProcessing: true };
  }

  return { category: 'general', type: 'general_query', requiresBasicRAG: true };
}
