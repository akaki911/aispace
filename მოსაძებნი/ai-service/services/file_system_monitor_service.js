...
class FileSystemMonitorService {
  ...
  async initialize() {
    ...
    try {
      ...
      if (disableWatchers) {
        console.warn('⚠️ [FILE MONITOR] File watchers disabled via configuration or test environment.');
    ...
    this.watcher
      ...
      .on('error', (error) => {
        ...
        if (error?.code === 'ENOSPC') {
          console.warn('⚠️ [FILE MONITOR] System file watcher limit reached. Disabling base monitor to protect the environment.');
  ...
  disableWatcher(reason = 'manual') {
    if (!this.watcher && !this.watcherActive) {
    ...
    if (watcherInstance && typeof watcherInstance.close === 'function') {
      watcherInstance.close().catch((closeError) => {
        console.warn('⚠️ [FILE MONITOR] Failed to close watcher cleanly:', closeError.message);
      });
    }

    console.warn(`⚠️ [FILE MONITOR] File watchers disabled (${reason}).`);
  }
  ...
  async stop() {
    ...
    this.watcherActive = false;
    this.watcherDisabledReason = 'stopped';
    this.isInitialized = false;
  ...
  getStatus() {
    return {
      ...
      isWatching: this.watcherActive && this.watcher !== null,
