...
function setEnhancedFileMonitorService(service) {
  ...
  if (service) {
    service.on('cycle:success', (data) => {
      console.log('🎯 [WEBSOCKET] Broadcasting cycle success event:', data);
      // Broadcast to all connected WebSocket clients
      broadcastToClients('cycle:success', data);
    });

    service.on('proposal:created', (data) => {
      console.log('📨 [WEBSOCKET] Broadcasting proposal created event:', data);
      broadcastToClients('proposal:created', data);
    });
  }
}

// WebSocket broadcasting function (mock for now)
function broadcastToClients(event, data) {
  // In a real implementation, this would use WebSocket server
  console.log(`📡 [BROADCAST] ${event}:`, data);

  // For now, we'll store events that frontend can poll
  global.liveAgentEvents = global.liveAgentEvents || [];
  global.liveAgentEvents.push({
    event,
    data,
  ...
  if (global.liveAgentEvents.length > 50) {
    global.liveAgentEvents = global.liveAgentEvents.slice(-50);
  }
}

// Events polling endpoint for frontend
