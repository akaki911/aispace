...
// Correlation ID middleware for AI Service
const correlationMiddleware = (req, res, next) => {
  ...
  res.send = function(body) {
    console.log(`[🤖✅ ${correlationId}] AI Service: ${req.method} ${req.path} - Response sent`, {
      correlationId,
  ...
  res.json = function(body) {
    if (res.statusCode >= 400) {
      console.error(`[🤖❌ ${correlationId}] AI Service: ${req.method} ${req.path} - Error response`, {
        correlationId,
        statusCode: res.statusCode,
        error: body,
        timestamp: new Date().toISOString(),
...
function logWithCorrelation(req, level, component, message, details = {}) {
  ...
  const logMessage = `[AI:${level.toUpperCase()}] [${correlationId}] [${component}] ${message}`;
