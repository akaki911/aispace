...
describe('sanitizeResponse Function Tests', () => {
  ...
  describe('flattenStructured helper', () => {
    test('should convert nested JSON payload into readable paragraphs', () => {
      const payload = {
        title: 'შედეგი',
        sections: [
          {
            title: 'Git ინსტრუქცია',
            bullets: [
              'git status ბრძანება ამოწმებს მიმდინარე სტატუსს',
              'git commit მხოლოდ მაშინ გამოიყენე როცა მზად ხარ'
            ],
            cta: 'გადადით: ტერმინალი → Git ოპერაციები'
          }
        ]
      };
      ...
      expect(flattened).toContain('Git ინსტრუქცია');
      expect(flattened).toContain('git status ბრძანება');
      expect(flattened).toContain('გადადით: ტერმინალი → Git ოპერაციები');
    });

    test('should fallback to JSON string when no textual fields exist', () => {
      const payload = {
        data: [{ id: 1 }, { id: 2 }]
      };
      ...
      expect(flattened).toContain('"id": 1');
      expect(flattened).toContain('"id": 2');
    });

    test('should return empty string for nullish payloads', () => {
      expect(flattenStructured(null)).toBe('');
      expect(flattenStructured(undefined)).toBe('');
    });
  });
  ...
  describe('Edge Cases', () => {
    ...
    test('should flatten structured payloads into paragraphs', () => {
      const payload = [
        {
          language: 'ka',
          sections: [
      ...
      const result = sanitizeResponse(payload);
