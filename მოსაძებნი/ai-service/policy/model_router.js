...
function routeQuery(query = '', opts = {}) {
  ...
  const REASONING_HINTS_RE =
    /(ამიხსენი|გაანალიზე|გაადათვალე|გადამიწყვიტე|დაგეგმე|იდეა(ები)?|why\b|how\s+to\s+solve|break\s*down|analysis|strategy|compare|evaluate|trade[-\s]*offs)/i;
  if (REASONING_HINTS_RE.test(qLower) || isLongOrComplex(q)) {
