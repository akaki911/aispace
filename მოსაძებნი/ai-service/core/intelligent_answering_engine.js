  ...
  try {
    ...
    console.log('🧾 [SEMANTIC SEARCH] Injecting trimmed knowledge snippets:',
      prioritizedChunks.map(chunk => ({
        id: chunk.id,
        source: chunk.source,
        chars: (chunk.content || chunk.snippet || chunk.text || '').length
      }))
    );
    ...
    for (const chunk of prioritizedChunks) {
      contexts.push({
        ...
        relevance: 0.6,
        content: `# 📚 Knowledge Base\n\n${chunk.content || chunk.snippet || chunk.text || ''}`,
        type: 'semantic',
        timestamp: Date.now()
      });
    }
...
function prioritizeKnowledgeChunks(chunks = [], limit = 2) {
  ...
  for (const chunk of chunks) {
    const origin = String(chunk?.originType || '').toLowerCase();
    const source = String(chunk?.source || '').toLowerCase();
    const isKnowledgeBase =
      origin.includes('knowledge') ||
      source.endsWith('.md') ||
      source.includes('faq') ||
      source.includes('doc');

    if (isKnowledgeBase) {
      knowledgeFirst.push(chunk);
    } else {
      fallback.push(chunk);
    }
  }

  return [...knowledgeFirst, ...fallback].slice(0, limit);
}
  ...
  try {
    ...
    if (budgetedFiles.length === 0 && semanticSearchService) {
      ...
      if (prioritizedKnowledge && prioritizedKnowledge.length > 0) {
        console.log('🧾 [FALLBACK] Injecting knowledge snippets:',
          prioritizedKnowledge.map((chunk, index) => ({
            rank: index + 1,
            id: chunk.id,
            source: chunk.source,
            chars: (chunk.content || chunk.snippet || chunk.text || '').length
        ...
        const sema = prioritizedKnowledge.map((t, i) => ({
          text: `# [KB FALLBACK:${i+1}] ${t.content || t.snippet || t.text || ''}`,
    ...
    if (semanticSearchService) {
      ...
      if (prioritizedKnowledge && prioritizedKnowledge.length > 0) {
        console.log('🧾 [FALLBACK:ERROR] Injecting knowledge snippets:',
          prioritizedKnowledge.map((chunk, index) => ({
            rank: index + 1,
            id: chunk.id,
            source: chunk.source,
  ...
  try {
    ...
    switch (model) {
      ...
      case 'small':
        ...
        response = await callSmallModelAPI(
          userMessage,
          conversationHistory,
          { ...options, policy, ragQuery, buildContext: buildEnhancedContext }
        );
        apiUsed = true;
        break;
      ...
      case 'large':
        ...
        // Use the enhanced buildEnhancedContext for context
        response = await callLargeModelAPI(
          userMessage,
          conversationHistory,
          { ...options, policy, ragQuery, buildContext: buildEnhancedContext }
        );
    ...
    if (typeof response === 'object' && response.content) {
      ...
      effectivePolicy = response.policy || policy;
    }

    // Calculate duration and final metadata
    const duration = Date.now() - startTime;
    const normalizedQueryType = typeof effectivePolicy === 'string' && effectivePolicy.trim().length > 0
      ? effectivePolicy.toLowerCase()
      : (policy || 'general').toLowerCase();
