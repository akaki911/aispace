...
// Role definitions
export type Role = 'SUPER_ADMIN' | 'ADMIN' | 'PROVIDER_ADMIN' | 'CUSTOMER';
...
export const PermissionsProvider: React.FC<PermissionsProviderProps> = ({ children }) => {
  ...
  const checkFirstBookingCompletion = async () => {
    ...
    try {
      ...
      if (completedBookings.size >= 1 && !user.hasCompletedFirstBooking) {
  ...
  const getAllRolePermissions = async (): Promise<RolePermissions[]> => {
    ...
    try {
      ...
      const managableRoles: Role[] = ['ADMIN', 'PROVIDER_ADMIN', 'CUSTOMER'];
