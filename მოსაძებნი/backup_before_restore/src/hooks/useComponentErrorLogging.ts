...
export const useComponentErrorLogging = (componentName: string) => {
  const { logError, logUIError } = useLogging();
  ...
  const errorHandlers = useMemo(() => ({
    ...
    logUIError: (action: string, error?: Error, details?: any) => {
      logUIError(componentName, action, {
        ...(error && {
    ...
    onClick: (handler: (event: React.MouseEvent) => void | Promise<void>, actionName?: string) => {
      ...
      return async (event: React.MouseEvent) => {
        try {
          ...
          logUIError(name, error as Error, {
            eventType: 'click',
    ...
    wrapAsync: async <T>(
      ...
      try {
        ...
        logUIError(operationName, error as Error, {
          operationType: 'async'
        });
        throw error;
      }
    }
  }), [componentName, logError, logUIError]);
