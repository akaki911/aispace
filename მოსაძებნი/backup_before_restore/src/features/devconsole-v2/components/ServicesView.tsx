...
export const ServicesView: React.FC<ServicesViewProps> = ({ onBackToLogs }) => {
  // 🇬🇪 Georgian Service Names
  const services: ServiceInfo[] = [
    {
      name: 'Frontend-Vite (დეველოპმენტი)',
      port: 3000,
      status: 'running',
      uptime: '2h 15m',
    ...
    {
      name: 'Frontend (ვებსაიტი)',
      port: 5000,
      status: 'running',
      uptime: '2h 15m',
      url: 'https://bakhmaro.replit.dev:5000',
      pid: 1236,
      cpu: 10,
      memory: 95
    },
    {
      name: 'Backend (სერვერი)',
      port: 5002,
      status: 'running',
      uptime: '2h 15m',
    ...
    {
      name: 'AI Service (ხელოვნური ინტელექტი)',
      port: 5001,
      status: 'running',
      uptime: '2h 15m',
          ...
          if (codeIndex.api_routes) {
            Object.entries(codeIndex.api_routes).forEach(([route, file]) => {
              ...
              routes.push({
                ...
                type: file.includes('ai-service') ? 'api' : 'backend'
              });
            });
          }
  ...
  const checkPortStatus = async (port: number): Promise<PortStatus> => {
    try {
      ...
      if (response.ok) {
        ...
        console.log(`✅ Port ${port} status:`, data);
        
        return {
          port,
          status: data.status === 'available' ? 'available' : (data.status === 'in_use' ? 'in_use' : 'conflict'),
          service: data.service || getServiceName(port),
          pid: data.pid,
          details: {
            command: data.command || data.service || 'Unknown Process',
        ...
        console.warn(`❌ Port ${port} API responded with status:`, response.status, response.statusText);
        // If rate limited or other non-JSON response, try to read as text for debugging
      ...
      console.warn(`🚨 Port status check failed for ${port}:`, error);
      // Additional context for debugging
      if (error instanceof Error && error.message.includes('JSON')) {
        console.warn(`💡 Likely rate limited - received HTML instead of JSON`);
      }
    }
  ...
  const handlePortConflictRecovery = async (conflictedPorts: number[]) => {
    try {
      ...
      // Show loading state
      setPortStatuses(prev => prev.map(p => 
        conflictedPorts.includes(p.port) 
          ? { ...p, status: 'cleaning' as any, details: { ...p.details, command: '🧹 მიმდინარე...' } }
          : p
      ));

      // Graceful shutdown first
      for (const port of conflictedPorts) {
        console.log(`🔄 ${getServiceName(port)}-ის graceful შეჩერება...`);
        await killPortProcess(port);
        await new Promise(resolve => setTimeout(resolve, 1000));
      }

      // Force cleanup if needed
      console.log('🔧 Force cleanup და port გათავისუფლება...');
      await fetch(`${API_BASE}/api/port-cleanup`, { method: 'POST' });

      // Wait for cleanup completion
      await new Promise(resolve => setTimeout(resolve, 3000));

      // Refresh port statuses
      await loadPortStatuses();

      console.log('✅ Port კონფლიქტები მოგვარებულია!');

    } catch (error) {
      console.error('❌ Port კონფლიქტის მოგვარება ვერ მოხერხდა:', error);
      // Reset to error state
      setPortStatuses(prev => prev.map(p => 
        conflictedPorts.includes(p.port) 
          ? { ...p, status: 'conflict', details: { ...p.details, command: '❌ შეცდომა' } }
          : p
      ));
    }
  };
  ...
  const loadPortStatuses = async () => {
    ...
    // Detect conflicts
    const conflicts = statuses.filter(s => s.status === 'conflict' || s.status === 'in_use');
  ...
  useEffect(() => {
    loadPortStatuses();
    const interval = setInterval(loadPortStatuses, 20000); // Reduced to 20s to avoid rate limiting
    return () => clearInterval(interval);
  }, []);
