...
export const calculateSeasonalPrice = (
  ...
  if (isFlexibleCottagePricing(pricing)) {
    const monthPricing = pricing[currentMonth];
    if (monthPricing) {
      const basePrice = (monthPricing.min + monthPricing.max) / 2;
      return Math.round(basePrice);
    }
  } else if (isFlexibleHotelPricing(pricing)) {
    const basePrice = pricing[currentMonth];
    if (basePrice) {
      return Math.round(basePrice);
    }
  } else if (isCottagePricing(pricing)) {
    const monthPricing = pricing[currentMonth as MonthKey];
    if (monthPricing) {
      const basePrice = (monthPricing.min + monthPricing.max) / 2;
      return Math.round(basePrice);
    }
  } else {
    const basePrice = pricing[currentMonth as MonthKey];
    if (basePrice) {
      return Math.round(basePrice);
    }
  }
