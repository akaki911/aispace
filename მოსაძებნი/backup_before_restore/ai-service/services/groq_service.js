...
// Ultra-fast model selection - 99% of queries use supported models
function selectOptimalModel(messages) {
  const userMessage = messages[messages.length - 1]?.content?.toLowerCase() || '';
  const messageLength = userMessage.length;

  // Only use 70B for extremely complex code analysis (1% of cases)
  const needsComplexAnalysis = messageLength > 200 && 
    /სრული\s*(სისტემის|პროექტის)\s*ანალიზი.*?(კოდი|არქიტექტურა)/.test(userMessage);

  // Updated to supported models - old models decommissioned
  return needsComplexAnalysis ? 'llama-3.3-70b-versatile' : 'llama-3.1-8b-instant';
}
