...
class EnhancedFileMonitorService {
  ...
  async initialize() {
    ...
    try {
      ...
      // Perform initial scan with analysis - DISABLED to prevent spam
  ...
  queueForAnalysis(filePath, trigger) {
    ...
    // console.log(`📊 [ENHANCED MONITOR] Queued ${filePath} for analysis (${trigger})`); // Disabled to reduce spam
  }
  ...
  startAnalysisProcessor() {
    ...
    }, 30000); // Process every 30 seconds - reduced frequency to prevent spam
  }
  ...
  async analyzeFile(filePath, trigger) {
    try {
      ...
      // console.log(`🔍 [ENHANCED MONITOR] Analyzing ${filePath}...`); // Disabled to reduce spam
