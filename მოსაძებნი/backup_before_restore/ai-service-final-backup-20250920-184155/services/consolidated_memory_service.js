...
// Use Firebase availability from global flag set by server.js
let isFirebaseAvailable = global.isFirebaseAvailable || false;

console.log(`🔥 Consolidated Memory Service: Firebase ${isFirebaseAvailable ? 'AVAILABLE' : 'NOT AVAILABLE'} - using ${isFirebaseAvailable ? 'Firebase' : 'local storage'}`);

// Only try to access Firebase if it's available
if (isFirebaseAvailable && admin.apps.length === 0) {
  console.warn('⚠️ Firebase should be available but no apps found - falling back to local storage');
