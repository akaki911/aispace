...
class ProposalBuilder {
    ...
    buildProposal(groupKey, evidence) {
        if (evidence.length === 0)
            return null;
    ...
    determineSeverity(category, evidence) {
        // P1: Security and critical performance issues
        if (category === 'security')
            return 'P1';
        if (category === 'performance' && evidence.some(e => e.note.includes('blocking') || e.note.includes('sync') || e.note.includes('memory'))) {
            return 'P1';
        }
    ...
    generateTitle(category, mainFile, evidence) {
    ...
    generateSummary(category, files, evidence) {
    ...
    getGeorgianSummary(category, count) {
        switch (category) {
            case 'security':
