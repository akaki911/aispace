...
class ToolRegistry {
  constructor() {
    ...
    this.gitService = require('../services/git_commands_service');
    
    this.initializeTools();
    console.log('Tool Registry initialized with Replit-style architecture + Git');
  }
  ...
  initializeTools() {
    ...
    this.registerTool('git_status', {
      ...
      inputs: [],
      outputs: ['repository_status'],
      sideEffects: false,
      handler: this.gitStatus.bind(this),
      failureModes: ['not_git_repo', 'permission_denied'],
      timeout: 5000
    });

    this.registerTool('git_add', {
      description: 'Stage files for commit',
      inputs: ['files'],
      outputs: ['staging_result'],
      sideEffects: true,
      handler: this.gitAdd.bind(this),
      failureModes: ['file_not_found', 'permission_denied'],
      timeout: 5000
    });

    this.registerTool('git_commit', {
      description: 'Commit staged changes',
      inputs: ['message', 'options'],
      outputs: ['commit_result'],
      sideEffects: true,
      handler: this.gitCommit.bind(this),
      failureModes: ['nothing_to_commit', 'permission_denied'],
      timeout: 10000
    });

    this.registerTool('git_push', {
      description: 'Push commits to remote repository',
      inputs: ['remote', 'branch'],
      outputs: ['push_result'],
      sideEffects: true,
      handler: this.gitPush.bind(this),
      failureModes: ['network_error', 'authentication_failed', 'push_rejected'],
      timeout: 30000
    });

    this.registerTool('git_pull', {
      description: 'Pull changes from remote repository',
      inputs: ['remote', 'branch'],
      outputs: ['pull_result'],
      sideEffects: true,
      handler: this.gitPull.bind(this),
      failureModes: ['network_error', 'merge_conflicts', 'authentication_failed'],
      timeout: 30000
    });

    this.registerTool('git_merge', {
      description: 'Merge branches with conflict detection',
      inputs: ['source_branch', 'options'],
      outputs: ['merge_result'],
      sideEffects: true,
      handler: this.gitMerge.bind(this),
      failureModes: ['merge_conflicts', 'branch_not_found'],
      timeout: 15000
    });
...
✨ *Bakhmaro AI Developer Assistant - Error Recovery Mode*`;
   ...
   */
  async gitStatus(inputs) {
    return await this.gitService.getStatus();
  }

  async gitAdd(inputs) {
    const { files = [] } = inputs;
    return await this.gitService.addFiles(files);
  }

  async gitCommit(inputs) {
    const { message, options = {} } = inputs;
    return await this.gitService.commit(message, options);
  }

  async gitPush(inputs) {
    const { remote = 'origin', branch } = inputs;
    return await this.gitService.push(remote, branch);
  }

  async gitPull(inputs) {
    const { remote = 'origin', branch } = inputs;
    return await this.gitService.pull(remote, branch);
  }

  async gitMerge(inputs) {
    const { source_branch, options = {} } = inputs;
    return await this.gitService.mergeBranch(source_branch, options);
  }
