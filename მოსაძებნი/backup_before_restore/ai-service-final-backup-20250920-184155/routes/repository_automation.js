...
router.get('/repository-automation/status', (req, res) => {
  ...
  try {
    ...
    const response = {
      ...
      status: {
        ...
        repoConfigured: !!((process.env.GITHUB_OWNER || process.env.GITHUB_REPO_OWNER) && (process.env.GITHUB_REPO || process.env.GITHUB_REPO_NAME))
      }
    };
    
    res.json(response);
  } catch (error) {
    console.error('Repository automation status error:', error);
    ...
    res.json({
      ...
      configuration: {
        valid: false,
        issues: [error.message || 'Configuration check failed']
      },
      capabilities: {
