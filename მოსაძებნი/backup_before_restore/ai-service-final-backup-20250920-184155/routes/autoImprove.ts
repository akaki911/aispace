...
import { discoveryRunner } from '../self-improvement/discoveryRunner';
import { proposalBuilder } from '../self-improvement/proposalBuilder';
...
router.post('/discover', async (req, res) => {
  try {
    ...
    const proposals = await proposalBuilder.buildProposals(discoveryResult.evidence);
    ...
    for (const proposal of proposals) {
      try {
        await backendClient.submitProposal(proposal);
        proposalsCreated++;
