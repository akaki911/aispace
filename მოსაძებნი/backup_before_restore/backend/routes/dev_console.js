...
router.get('/console/stream', (req, res) => {
  ...
  const logInterval = setInterval(() => {
    ...
    for (let i = 0; i < numLogs; i++) {
      ...
      const message = georgianMessages[Math.floor(Math.random() * georgianMessages.length)];
...
router.post('/commands/restart', (req, res) => {
  ...
  const responses = {
    'ai-service': { port: 5001, status: 'restarting', message: 'AI Service restart initiated' },
...
router.get('/services/status', (req, res) => {
  const services = [
    { name: 'AI Service', slug: 'ai-service', port: 5001, status: 'running', pid: Math.floor(Math.random() * 9000) + 1000, uptime: '2h 15m', url: 'https://bakhmaro.replit.dev:5001' },
    ...
    { name: 'Frontend', slug: 'frontend', port: 3000, status: 'running', pid: Math.floor(Math.random() * 9000) + 1000, uptime: '2h 15m', url: 'https://bakhmaro.replit.dev:3000' }
  ];
...
router.get('/metrics', (req, res) => {
  const metrics = {
    ...
    processes: [
      { name: 'ai-service', pid: 1234, cpu: Math.random() * 30 + 10, memory: Math.random() * 50 + 150, status: 'running' },
...
module.exports = {
  ...
  logBuffer,
  metricsBuffer,
  responseSizeLimiter // Export for potential use elsewhere
};