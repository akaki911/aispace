...
```text
Converting ESM imports to CommonJS format.
```
...
• vite.config.ts - Build კონფიგურაცია
  ...
  getPreBuiltContextForFiles: (fileTypes) => {
    const fileContexts = {
      'booking': {
        ...
        keyFeatures: ['რეალ-დროის ხელმისაწვდომობა', 'ავტომატური ფასების გამოთვლა', 'კალენდრის ინტეგრაცია'],
