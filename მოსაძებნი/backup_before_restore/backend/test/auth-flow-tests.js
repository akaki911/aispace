...
class AuthFlowTests {
  ...
  async testWebAuthnRegistrationOptions() {
    ...
    if (process.env.NODE_ENV === 'development') {
      ...
      assert(response.body.challenge, 'Should include WebAuthn challenge');
      assert(response.body.user, 'Should include user information');
    }
  }
  ...
  async testDevModeBypass() {
    ...
    const response = await request(this.app)
      .post('/api/admin/auth/webauthn/verify')
      .send({ assertion: 'DEV_MODE', personalId: '01019062020' });

    assert(response.status === 200, 'Dev mode bypass should work');
    assert(response.body.ok === true, 'Response should indicate success');
  ...
  async testAuthenticationPersistence() {
    ...
    assert(response.body.ok === true, 'Should be authenticated with valid session');
  ...
  async testLogoutFlow() {
    ...
    assert(response.body.ok === true, 'Logout should succeed');
  ...
  async testUnauthorizedAccess() {
    const response = await request(this.app)
      .post('/api/admin/auth/webauthn/register/options')
      .send({ personalId: '12345678901' }); // Wrong personal ID
