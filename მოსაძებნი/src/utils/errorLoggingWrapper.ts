...
export function withErrorLogging<P extends Record<string, any>>(
  ...
  const ErrorLoggedComponent = forwardRef<any, P>((props, ref) => {
    ...
    return createElement(WrappedComponent, { ...(wrappedProps as P), ref });
  });
...
export function logSyncErrors<T extends (...args: any[]) => any>(
  ...
  return ((...args: any[]) => {
    try {
      ...
      void globalErrorHandler.logError(error as Error, `${componentName}.${operationName}`);
      throw error; // Re-throw to maintain original behavior
    }
  }) as T;
}
    ...
    try {
      ...
      if (result instanceof Promise) {
        return result.catch((error: Error) => {
          void globalErrorHandler.logError(error, `${target.constructor.name}.${propertyName}`);
          throw error;
        });
      }
      ...
      void globalErrorHandler.logError(error as Error, `${target.constructor.name}.${propertyName}`);
      throw error;
    }
  };
}
