...
const getErrorDetails = (error: unknown): { message: string; code?: string } => {
  ...
  if (typeof error === 'object' && error !== null) {
    ...
    return {
      message: typeof maybe.message === 'string'
        ? maybe.message
...
export const messagingService = {
  ...
  async sendMessage(conversationId: string, content: string, messageType: 'text' | 'image' | 'file' = 'text', attachments: string[] = []): Promise<Message> {
    try {
      ...
      if (code === 'permission-denied') {
        ...
        throw new Error(`შეცდომა შეტყობინების გაგზავნისას: ${message || 'უცნობი შეცდომა'}`);
      }
    }
  },
  ...
  async deleteMessage(conversationId: string, messageId: string): Promise<void> {
    try {
      ...
      throw new Error(`შეცდომა შეტყობინების წაშლისას: ${message}`);
    }
  },
  ...
  async archiveConversation(conversationId: string): Promise<void> {
    try {
      ...
      const { message } = getErrorDetails(error);
      throw new Error(`შეცდომა საუბრის არქივირებისას: ${message}`);
    }
  },
  ...
  async restoreConversation(conversationId: string): Promise<void> {
    try {
      ...
      const { message } = getErrorDetails(error);
      throw new Error(`შეცდომა საუბრის აღდგენისას: ${message}`);
    }
  },
  ...
  async deleteConversation(conversationId: string): Promise<void> {
    try {
      ...
      const { message } = getErrorDetails(error);
      throw new Error(`შეცდომა საუბრის წაშლისას: ${message}`);
    }
  },
  ...
  async updateConversationPriority(conversationId: string, priority: 'low' | 'medium' | 'high'): Promise<void> {
    try {
      ...
      const { message } = getErrorDetails(error);
      throw new Error(`შეცდომა პრიორიტეტის განახლებისას: ${message}`);
    }
  },
  ...
  async addConversationTags(conversationId: string, tags: string[]): Promise<void> {
    try {
      ...
      const { message } = getErrorDetails(error);
      throw new Error(`შეცდომა ტეგების დამატებისას: ${message}`);
    }
  },
  ...
  async leaveConversation(conversationId: string): Promise<void> {
    try {
      ...
      const { message } = getErrorDetails(error);
      throw new Error(`შეცდომა საუბრიდან გასვლისას: ${message}`);
    }
  },
  ...
  async editMessage(conversationId: string, messageId: string, newContent: string): Promise<void> {
    try {
      ...
      const { message } = getErrorDetails(error);
      throw new Error(`შეცდომა შეტყობინების რედაქტირებისას: ${message}`);
    }
  },
  ...
  async pinMessage(conversationId: string, messageId: string, isPinned: boolean): Promise<void> {
    try {
      ...
      const { message } = getErrorDetails(error);
      throw new Error(`შეცდომა პინის სტატუსის განახლებისას: ${message}`);
    }
  },
  ...
  async reportMessage(conversationId: string, messageId: string, reason: string): Promise<void> {
    try {
      ...
      const { message } = getErrorDetails(error);
      throw new Error(`შეცდომა შეტყობინების რეპორტისას: ${message}`);
    }
  },
  ...
  async forwardMessage(fromConversationId: string, messageId: string, toConversationId: string): Promise<void> {
    try {
      ...
      const { message } = getErrorDetails(error);
      throw new Error(`შეცდომა შეტყობინების გადაგზავნისას: ${message}`);
    }
  },
  ...
  async blockUser(userId: string): Promise<void> {
    try {
      ...
      const { message } = getErrorDetails(error);
      throw new Error(`შეცდომა მომხმარებლის დაბლოკვისას: ${message}`);
    }
  },
  ...
  async muteConversation(conversationId: string, isMuted: boolean): Promise<void> {
    try {
      ...
      const { message } = getErrorDetails(error);
      throw new Error(`შეცდომა გაუჩუმების სტატუსის განახლებისას: ${message}`);
    }
  }
};
