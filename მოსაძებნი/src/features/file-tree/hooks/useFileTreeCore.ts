...
const isStorageManagedPath = (filePath: string): boolean => filePath.startsWith(STORAGE_PREFIX);

const isBinaryPath = (filePath: string): boolean => {
  const ext = getExtension(filePath);
  return isStorageManagedPath(filePath) || (ext ? BINARY_FILE_EXTENSIONS.has(ext) : false);
};

const isTextPath = (filePath: string): boolean => {
  if (isStorageManagedPath(filePath)) {
    return false;
  }
...
export const useFileTreeCore = (props?: FileTreeProps) => {
  ...
  const closeTab = useCallback((tabId: string) => {
    setOpenTabs(prev => {
      ...
      if (closingTab?.binaryUrl && closingTab.binaryUrl.startsWith('blob:')) {
        URL.revokeObjectURL(closingTab.binaryUrl);
      }
  ...
  const loadTabContent = useCallback(async (tab: Tab) => {
    ...
    const expectsText = isTextPath(tab.path);
    const expectsBinary = !expectsText || isBinaryPath(tab.path);
    ...
    try {
      ...
      if (expectsBinary) {
        ...
        console.log(`✅ Binary content loaded for tab: ${tab.path}`);
      } else {
        const content = await response.text();

        setOpenTabs(prev => prev.map(t =>
          t.id === tab.id
            ? { ...t, content, binaryUrl: undefined, contentType: 'text/plain', hasUnsavedChanges: false, isBinary: false }
            : t
        ));
