...
const ReplitInterface: React.FC = () => {
      ...
      try {
        ...
        setOpenFiles(prev => prev.map(f =>
          f.path === path
            ? {
                ...f,
                error: message || 'ფაილის ჩატვირთვა ვერ მოხერხდა',
                loading: false
              }
            : f
        ));
        addLog('error', `ფაილის ჩატვირთვა ვერ მოხერხდა: ${fileName} - ${message}`, 'ფაილური ვიუერი');
      }
    } else {
      toggleNode(path);
    }
  ...
  const performFileSearch = async (query: string) => {
    ...
    try {
      ...
      console.log(`🔍 [File Search] Making request to: /api/files/search?q=${encodeURIComponent(limitedQuery)}&limit=50`);

      const response = await fetch(`/api/files/search?q=${encodeURIComponent(limitedQuery)}&limit=50`, {
        credentials: 'include'
      });
      ...
      if (Array.isArray(data)) {
        const formattedResults: SearchResult[] = data.map((result: any) => ({
          ...
          matches: Array.isArray(result.lines) ? result.lines : []
        }));
      ...
      const message = error instanceof Error ? error.message : String(error);
      setSearchResults([]);
      addLog('error', `ძებნის შეცდომა: ${message}`, 'ფაილების ძებნა');
    } finally {
  ...
  const getRecentFiles = async () => {
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : String(error);
      addLog('error', `તાજેતრის ფაილების შეცდომა: ${message}`, 'ფაილების ძებნა');
  ...
  const getCategoryFiles = async (category: string) => {
    setSearchLoading(true);
    try {
      const response = await fetch(`/api/files/category/${category}?limit=20`, {
        credentials: 'include'
      });
      ...
      const message = error instanceof Error ? error.message : String(error);
      addLog('error', `კატეგორიის ფაილების შეცდომა: ${message}`, 'ფაილების ძებნა');
      // Provide fallback categories based on file system
      const fallbackCategories: Record<string, SearchResult[]> = {
        'backup': [
        ...
        'config': [
          ...
          { name: 'vite.config.mts', path: 'vite.config.mts', type: 'config', tag: 'config', size: 512, modified: new Date().toISOString(), directory: '.' }
        ]
      };
