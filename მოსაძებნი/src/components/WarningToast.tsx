...
import { X, AlertTriangle, AlertCircle, CheckCircle, Info, ChevronDown, ChevronUp, Globe } from 'lucide-react';
...
interface WarningToastProps {
  ...
  messages: string[];
  title?: string;
  type?: 'warning' | 'error' | 'success' | 'info' | 'network' | 'validation';
...
const WarningToast: React.FC<WarningToastProps> = ({
  ...
  const getColors = () => {
    switch (type) {
      ...
      case 'success':
        return 'bg-green-50 border-green-200 text-green-800 dark:bg-green-900 dark:bg-opacity-20 dark:border-green-800 dark:text-green-200';
      case 'info':
              ...
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center">
                  ...
                  <div className="ml-3">
                    ...
                    {httpStatus && (
                      <p className="text-sm opacity-75">HTTP {httpStatus}</p>
                    )}
                    {component && (
                      <p className="text-xs opacity-60">კომპონენტი: {component}</p>
                    )}
                  </div>
                </div>
                <button
