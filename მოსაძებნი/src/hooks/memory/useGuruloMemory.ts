...
export const useGuruloMemory = (): GuruloMemoryHook => {
  ...
  const handleWebSocketMessage = useCallback((message: any) => {
    switch (message.type) {
      ...
      case 'connected':
        ...
        break;
      case 'heartbeat':
        console.debug('💓 [Gurulo Memory] Heartbeat received');
        break;
      default:
        console.debug('ℹ️ [Gurulo Memory] Unhandled realtime event:', message);
        break;
    }
  }, [updateContext, rememberFact, setWsStatus]);
  ...
  const connectWebSocket = useCallback(() => {
    ...
    try {
      ...
      const wsUrl = `${protocol}//${window.location.host}/api/gurulo/ws`;
      ...
      socket.onerror = (error) => {
        console.error('❌ [Gurulo Memory] WebSocket connection error:', error);
      };
