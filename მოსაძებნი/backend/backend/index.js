...
const DEFAULT_SERVICE_PERMISSIONS = ['chat', 'models', 'fs'];
const SERVICE_AUTH_CONFIG = getServiceAuthConfigs()[0] || null;

if (SERVICE_AUTH_CONFIG?.isFallback) {
  console.warn('⚠️ [Backend] Using fallback service auth secret. Configure AI_SERVICE_SHARED_SECRET for secure environments.');
}

const generateCorrelationId = () => `backend-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 8)}`;

const getServiceToken = (permissions = DEFAULT_SERVICE_PERMISSIONS) => {
  try {
    ...
    console.error('❌ [Backend] Failed to generate service token', { message: error.message });
    return null;
  }
};

const buildServiceHeaders = (req, permissions = DEFAULT_SERVICE_PERMISSIONS) => {
  const token = getServiceToken(permissions);
...
app.use('/api/fs', (req, res) => {
  ...
  const headers = buildServiceHeaders(req, ['fs', 'chat']);
