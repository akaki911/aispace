...
function shouldExclude(filePath, fileName) {
  ...
  const protectedPathSegments = filePath.split(path.sep);
  if (protectedPathSegments.some(segment => PROTECTED_DIRS.includes(segment))) {
    return true;
  }
  ...
  // Only exclude specific sensitive hidden files, allow most dotfiles
  const sensitiveHiddenFiles = [
    '.env', '.env.local', '.env.production', '.env.development',
    '.firebase', '.firebaserc', '.gitsecret', '.ssh',
    '.netrc', '.npmrc', '.aws', '.docker',
    'firebase-adminsdk.json'
  ];
  
  if (fileName.startsWith('.') && sensitiveHiddenFiles.includes(fileName)) {
    return true;
  }
  
  // Exclude hidden directories that are typically cache/temp
  const hiddenPathSegments = filePath.split(path.sep);
  const hiddenDirs = ['.cache', '.tmp', '.temp', '.DS_Store', '.Spotlight-V100', '.Trashes'];
  if (hiddenPathSegments.some(segment => hiddenDirs.includes(segment))) {
    return true;
  }
